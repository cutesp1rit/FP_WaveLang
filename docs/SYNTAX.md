...
### Операции волновой композиции

**Синтаксис:** 
- `x ~~ f ~~ g` - строгая композиция
- `x ~> f ~> g` - ленивая композиция
- `[a, b, ...] |~ f |~ g` - параллельная композиция

**Описание:** При строгой композиции все элементы вычисляются сразу, при ленивой - при необходимости, параллельная применяет функцию поэлементно к списку.
...
// Ленивая композиция (на данном этапе ведёт себя как строгая):
3 ~> double ~> addOne     // Результат: 7


// Параллельная композиция:
[1, 2, 3] |~ double |~ addOne   // Результат: [3, 5, 7]
// 1 -> double(1)=2 -> addOne(2)=3
// 2 -> double(2)=4 -> addOne(4)=5
// 3 -> double(3)=6 -> addOne(6)=7
```
